	...
	case "build":
		...
		for i := range repositoriesSlice {
			if bucketName != credentials[i].AdditionalProperties.TrivyVulnerabilityDBGCSBucket {
				...
				foundation.RunCommandWithArgs(ctx, "/google-cloud-sdk/bin/gcloud", []string{"auth", "application-default", "login"})
				foundation.RunCommandWithArgs(ctx, "/google-cloud-sdk/bin/gsutil", []string{"-m", "cp", "-r", fmt.Sprintf("gs://%v/trivy-cache/*", bucketName), "/trivy-cache"})
			}
		}
